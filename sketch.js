...
function draw() {
  ...
  // טווח התנועה של המחבט של ״השחקן״ על ציר האיקס
...
//הצל של הכדור
  ...
  if (playerScore > computerScore) {
    text("AUSTRALIAN OPEN", 210 + shadowOffset, 65 + shadowOffset);
  }

  fill(textColor);
  text("AUSTRALIAN OPEN", 210, 60);
...
function drawCountdown() {
  ...
  fill(137, 153, 64);
  textSize(35);
  text(hours + ":" + nf(minutes, 2) + ":" + nf(seconds, 2), 470, 735);
}
...
function drawScores() {
  ...
  fill(137, 153, 64);
  textSize(35);
  text("   : P" + playerScore, 500, 65);
  text("C" + computerScore, 470, 65);
}
...
function resetGame() {
  playerPaddle = createVector(width / 2, 622);
  computerPaddle = createVector(width / 2, 167);
  playerScore = 0;
  computerScore = 0;
  resetBall();
}
